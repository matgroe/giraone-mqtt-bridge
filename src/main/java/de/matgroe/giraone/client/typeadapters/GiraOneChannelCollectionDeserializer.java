...
/**
 * Uses the Gson parsing to deserializes the response of {@link
 * de.matgroe.giraone.client.GiraOneCommand} command {@link GetDiagnosticDeviceList} into {@link
 * GiraOneChannelCollection}.
 *
 * @author Matthias Gröger - Initial contribution
 */
public class GiraOneChannelCollectionDeserializer
    implements JsonDeserializer<GiraOneChannelCollection> {

  @Override
  public GiraOneChannelCollection deserialize(
    ...
    if (jsonElement.isJsonArray()) {
      ...
      List<JsonObject> channelLocations =
          jsonElement.getAsJsonArray().asList().stream()
              ...
              .filter(
                  e ->
                      e.has(PROPERTY_MAINTYPE)
                          && "Root".equals(e.get(PROPERTY_MAINTYPE).getAsString()))
              .findFirst()
              .stream()
              .map(this::streamChannelLocationInformation)
              .map(Stream::toList)
      ...
      jsonElement.getAsJsonArray().asList().stream()
          .filter(JsonElement::isJsonObject)
          .map(JsonElement::getAsJsonObject)
          .filter(e -> e.has(PROPERTY_CHANNEL_VIEW_URN))
  ...
  private JsonObject enrichLocation(
      JsonDeserializationContext jsonDeserializationContext,
      JsonObject jsonObject,
      List<JsonObject> channelLocations) {
    JsonElement viewId = jsonObject.get(PROPERTY_CHANNEL_VIEW_ID);
    if (viewId != null) {
      channelLocations.stream()
          .filter(f -> f.has(PROPERTY_CHANNEL_VIEW_ID))
          .filter(f -> f.get(PROPERTY_CHANNEL_VIEW_ID).getAsString().equals(viewId.getAsString()))
          .findFirst()
  ...
  private GiraOneChannel createGiraOneChannel(
      JsonDeserializationContext jsonDeserializationContext, JsonElement jsonElement) {
  ...
  private Stream<JsonElement> streamChannelLocationInformation(JsonObject jsonObject) {
    ...
    if (jsonObject.has(PROPERTY_CONTENT)) {
      jsonElements =
          Stream.concat(
              jsonElements,
              streamChannelLocationInformation(
                  jsonObject, jsonObject.getAsJsonArray(PROPERTY_CONTENT)));
    }
    return jsonElements;
  }

  private Stream<JsonElement> streamChannelLocationInformation(
      JsonObject jsonParentObject, JsonArray jsonArray) {
    Stream<JsonElement> jsonElements = Stream.empty();
        ...
        if (jsonObject.has(PROPERTY_MAINTYPE)) {
          ...
          if (jsonParentObject.has(PROPERTY_NAME)) {
            jsonObject.addProperty(
                PROPERTY_LOCATION, jsonParentObject.get(PROPERTY_NAME).getAsString());
          }
